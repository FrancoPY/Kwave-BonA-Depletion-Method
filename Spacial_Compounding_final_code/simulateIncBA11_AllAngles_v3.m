...
a_bg  = 0.10;   y_bg  = 2.00;   % 0.1*f^2
a_inc = 0.10;   y_inc = 2.00;   % misma atenuación en la inclusión
...
medium.BonA  = BonA_bg * ones(nx, ny);
...
medium.BonA        = medium.BonA + (BonA_inc - BonA_bg) * inclusion;
...
% Ãndice de elementos (centrado en cero)
...
for steering_angle = steeringAngles
    ...
    fprintf('=== Ãngulo de steering: %d grados ===\n', steering_angle);

    % Offset base y retardo por elemento (dependen del Ã¡ngulo)
